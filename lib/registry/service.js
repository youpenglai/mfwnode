...
class Service {
  ...
  discover(serviceName) {
    if (!serviceName) {
      return Promise.reject({ status: 400, message: 'not serviceName' });
    }
    const query = {
    ...
    if (utils.getCache(serviceName)) {
      ...
      if (serviceNode) {
        return Promise.resolve(serviceNode);
      }
      return Promise.reject({ status: 500, message: 'cache data is empty' })
    }
    const url = this.consul.baseUrl + query.path;
